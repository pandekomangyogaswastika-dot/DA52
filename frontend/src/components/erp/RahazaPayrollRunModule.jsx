...
function GLStatusBadge({ run, onNavigate }) {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {run.gl_je_number && (
        <span
          className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 cursor-pointer hover:bg-emerald-500/20"
      ...
      {run.payment_gl_je_number && (
        <span
          className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded font-medium bg-blue-500/10 text-blue-400 border border-blue-500/30 cursor-pointer hover:bg-blue-500/20"
      ...
      {run.post_error && !run.gl_je_number && (
        <span className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded font-medium bg-red-500/10 text-red-400 border border-red-500/30" title={run.post_error}>
...
export default function RahazaPayrollRunModule({ token, onNavigate }) {
  ...
  // Payment dialog
  const [payDialog, setPayDialog] = useState(null); // run object
  ...
  const finalizeRun = async (id) => {
    ...
    if (r.ok) { toast.success('Run difinalisasi & jurnal GL dibuat'); fetchRuns(); if (viewing?.run?.id === id) openRun(id); }
                  ...
                  <tr key={r.id} className="border-t border-[var(--glass-border)] hover:bg-[var(--glass-bg-hover)] transition-colors" data-testid={`pr-row-${r.run_number}`}>
                    ...
                    <td className="px-3 py-2 text-right whitespace-nowrap">
                      ...
                      {r.status === 'finalized' && r.post_error && (
                        <button onClick={() => retryPost(r.id)} className="text-xs text-amber-300 hover:underline" title="Retry GL Posting"><AlertTriangle className="w-3 h-3 inline" /></button>
                      )}
                      ...
                      {r.status === 'finalized' && r.gl_je_number && r.payment_status !== 'paid' && (
                        <button
                          ...
                          className="text-xs text-blue-300 hover:underline ml-1"
                        ...
                        <>
                          {r.bpjs_payment_status !== 'paid' && (
                            <button
                              ...
                              className="text-xs text-teal-300 hover:underline ml-1"
                              title="Bayar BPJS"
                          ...
                          {r.pph21_payment_status !== 'paid' && (
                            <button
                              ...
                              className="text-xs text-purple-300 hover:underline ml-1"
      ...
      <Dialog open={!!payDialog} onOpenChange={o => !o && setPayDialog(null)}>
        <DialogContent className="max-w-sm" data-testid="pay-dialog">
          ...
          <div className="space-y-3 mt-2">
            {payDialog && (
              <div className="rounded-lg bg-blue-500/10 border border-blue-500/30 px-3 py-2 text-sm">
        ...
        <table className="w-full text-sm">
          <thead className="bg-[var(--glass-bg)]">
            <tr className="text-left text-xs text-muted-foreground">
              ...
              <th className="px-3 py-2 text-right">Bonus Prod.</th>
          ...
          <tbody>
            {payslips.map(s => (
              <tr key={s.id} className="border-t border-[var(--glass-border)] hover:bg-[var(--glass-bg-hover)] transition-colors" data-testid={`slip-${s.employee_code}`}>
                ...
                <td className="px-3 py-2 text-right font-mono text-xs">{fmtIDR(s.base_salary)}</td>
                <td className="px-3 py-2 text-right font-mono text-xs">{fmtIDR(s.transport_allowance)}</td>
                <td className="px-3 py-2 text-right font-mono text-xs">{fmtIDR(s.meal_allowance)}</td>
                <td className="px-3 py-2 text-right font-mono text-xs">{fmtIDR(s.production_bonus)}</td>
                <td className="px-3 py-2 text-right font-mono text-xs">{fmtIDR(s.overtime_pay)}</td>
                <td className="px-3 py-2 text-right font-mono text-xs text-red-300">{fmtIDR(s.total_deductions)}</td>
                <td className="px-3 py-2 text-right font-mono text-xs font-bold text-emerald-300">{fmtIDR(s.net_salary)}</td>
