...
async def _get_employee_for_user(db, user_id: str):
    """
    Return (user, employee) doc linked to user_id.
    Cek urutan:
      1. rahaza_employees.user_id == user_id  (link baru via link-user endpoint)
      2. users.employee_id → rahaza_employees  (link lama via admin endpoint)
      3. rahaza_employees.email == user.email  (fallback email match)
    """
    user = await db.users.find_one({"id": user_id}, {"_id": 0})
    if not user:
        return None, None

    # 1. Cek via rahaza_employees.user_id (link baru)
    ...
    # 2. Cek via users.employee_id (link lama)
